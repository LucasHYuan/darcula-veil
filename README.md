...
`feFuncX type="discrete"` 的语义是「输入 0 取表首，输入 1 取表尾」。色带按亮度升序排列时：

| 源页面 | Reverse mapping | 理由 |
|---|---|---|
| **亮底**（绝大多数网页） | **开**（默认） | 白底亮度≈1 会落到色带末端即最亮色，整页变浅灰，比原页面还扎眼。必须反向 |
| **暗底**（阅读器夜间模式等） | **关** | 页面本就是暗底亮字，反向会把暗底送到色板亮端，页面反而变浅、文字被压暗，整个反过来 |

因此渐晕的边缘取色也要跟着走：反向时取 `palette.last()`，不反向时取 `palette.first()`——两者都是「页面背景最终会变成的那个颜色」，渐晕必须与它一致才看不出接缝。

渐晕本身对两种呈现模式都可用，实现收在 `VeilVignette.wrap()` 里：把滤镜从 `html` 下移到 `body`，渐晕挂 `html::after`，并显式给 `html` 填背景色。连续滤镜模式下边缘取 `UIUtil.getPanelBackground()`。
